...
### Changed
- **`seeds/tasks.rb`**:
  - **`actual_time`** can no longer be **NaN**—playtime is over.
- **`models/goal.rb`**:
  - **Destroying a goal now removes its related tasks**
- **`goals/show.html.erb`**:
  - Aligned styling with **tasks/show** for consistency.
  - Added **emoji-powered** sub-nav for extra flair.
- **`tasks/show.html.erb`**:
  - Now rocking the same **sub-nav styling** as goals.
...
- **Stronger data integrity** (no more NaN nonsense).  
